...
(async () => {
  ...
  check('GET / → scrollbar-gutter:stable (kein Karten-Shift beim Tab-Wechsel)', idx.status === 200 && /html \{[^}]*scrollbar-gutter:stable/.test(idxText));
  check('GET / → Video-Player ausgeliefert (Toggle + video-Element + i18n)', idx.status === 200 && idxText.includes('mediaKindToggle') && idxText.includes('playerVideo') && idxText.includes('player.kindAudio') && idxText.includes('player.kindVideo') && idxText.includes('player.emptyVideo') && idxText.includes('currentMedia'));
